...
const initializeInputStochRSI = async () => {
  ...
  for (
    let i = 0, currentClosePrice = null;
    i <= STOCHRSI_CALCULATION_PERIOD + 3;
    i++
  ) {
    currentClosePrice =
      candles[candles.length - 1 - (STOCHRSI_CALCULATION_PERIOD + 3) + i].close;
    inputStochRSI.values[i] = Number(currentClosePrice);
  }
  console.log("inputStochasticRSI: ", inputStochRSI);
  console.log(
    "inputStochasticRSI.value.lenght: ",
    inputStochRSI.values.length,
    "\n"
  );
};
...
const initializeSmoother = async (SRSI) => {
  ...
  filter[2] =
    c1 * (SRSI[3].stochRSI + SRSI[2].stochRSI) +
    c2 * filter[1] +
    c3 * filter[0];
...
(async function main() {
  let calculatedStochRSI = null,
    smoothedStochRSI = null,
    buySuccess = null;
  try {
